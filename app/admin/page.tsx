...
export default function CrmDashboardPage() {
  ...
  // STRICTLY GATED: Only managers and super admins are ever allowed to preview or open other dashboards.
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (!isManager) {
      if (viewAs) setViewAs(null);
  ...
  }, [isManager, realRole, viewAs, basePath, router]);
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f7fb] text-[#14213d]">
      ...
      <aside className="w-64 bg-white border-r border-[#e4e9f1] p-4 flex flex-col justify-between shrink-0">
        ...
        {/* Sidebar Footer: Role info & Prominent Logout */}
        <div className="pt-4 mt-4 border-t border-[#e4e9f1] flex flex-col gap-2">
          <div className="px-2">
            <div className="text-xs font-bold text-slate-900 truncate">
              {username || "Logged in"}
            </div>
            <div className="text-[11px] font-semibold text-[#001f97] truncate">
              {roleLabel}
            </div>
          </div>
          <button
            onClick={logout}
            disabled={loggingOut}
            className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold text-rose-600 hover:bg-rose-50 border border-rose-200/70 transition-colors disabled:opacity-50 w-full"
        ...
        <header className="h-20 bg-white border-b border-[#e4e9f1] px-6 flex items-center justify-between gap-4 sticky top-0 z-10 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={logout}
              disabled={loggingOut}
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold text-rose-600 bg-rose-50/60 border border-rose-200 hover:bg-rose-100 transition-colors"
              title="Sign out of your account"
            >
