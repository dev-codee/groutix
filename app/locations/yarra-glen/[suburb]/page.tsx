...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Shower Regrouting ${name} | Yarra Glen | GROUTIX`,
    description: `Specialist shower regrouting, grout repair and leaking shower services in ${name}, Yarra Glen. Professional wet-area repairs backed by a 10-year waterproof guarantee.`,
    alternates: { canonical: `/locations/yarra-glen/${suburb}` },
    openGraph: {
      title: `Shower Regrouting ${name} | Yarra Glen — GROUTIX`,
      description: `Local shower regrouting, grout repair and leaking shower solutions in ${name}, Yarra Glen. Free quotes and specialist workmanship.`,
      url: `/locations/yarra-glen/${suburb}`,
      type: "website",
    },
  };
}
...
export default async function SuburbPage({ params }: Props) {
  ...
  const serviceAreaText = `GROUTIX provides shower regrouting, grout repair, silicone replacement and leaking shower repairs across ${suburbName} and the greater Yarra Glen region. We target failed grout joints and worn seals without turning a repair into a full renovation.
  ...
  return (
    <>
      ...
      <CityPageClient
        ...
        heroSubtitle={`Professional shower regrouting, grout repair and leaking shower services in ${suburbName}, Yarra Glen. GROUTIX restores wet areas without unnecessary tile removal and backs full shower regrouting with a 10-year waterproof guarantee.`}
