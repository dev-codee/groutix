              ...
              {payStep === "ask_type" && (
                <div className="rounded-xl border border-[#001f97]/30 bg-blue-50 p-2.5 space-y-2 shadow-sm">
                    ...
                    <div className="text-[10px] font-semibold text-slate-600 text-center">
                      ...
                      {amountPaid !== null && amountPaid > 0 && (
                        <div className="text-[9.5px] text-amber-800 mt-0.5">
                          ...
                          {remainingAmt !== null && (
                            <span> | Rem: <b className="text-rose-700">AUD ${remainingAmt.toFixed(2)}</b></span>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    ...
                    >
                      ✅ {amountPaid && amountPaid > 0 && remainingAmt !== null && remainingAmt > 0 ? "Pay Rem." : "Full"}
                ...
                <div className="rounded-xl border border-amber-300 bg-amber-50 p-2.5 space-y-2 shadow-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-amber-700 uppercase tracking-wider">
                      {amountPaid && amountPaid > 0 ? "Add Amount (AUD)" : "Amount (AUD)"}
                    </span>
                  ...
                  {amountPaid !== null && amountPaid > 0 && (
                    <div className="text-[9.5px] font-medium text-slate-600 bg-white/80 p-1 rounded border border-amber-200">
                      Paid: <b className="text-emerald-700">AUD ${amountPaid.toFixed(2)}</b>
                      {remainingAmt !== null && (
                        <span> • Rem: <b className="text-rose-700">AUD ${remainingAmt.toFixed(2)}</b></span>
                      )}
                    </div>
                  ...
                  {partialAmt && !isNaN(parseFloat(partialAmt)) && (
                    <div className="text-[10px] font-bold text-slate-700 bg-amber-100/60 p-1.5 rounded border border-amber-200/80">
                      {amountPaid && amountPaid > 0 ? (
                        <>
                          New Total: <span className="text-emerald-700 font-black">AUD ${((amountPaid || 0) + parseFloat(partialAmt)).toFixed(2)}</span>
                          {invoiceTotal && (
                  ...
                  >
                    {amountPaid && amountPaid > 0
                      ...
                      : `Confirm Partial (AUD $${parseFloat(partialAmt || "0").toFixed(2)})`}
                  </button>
                </div>
              )}
            </div>
