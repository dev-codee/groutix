...
export function TechnicianLeadRow({ l }: { l: Lead }) {
  ...
  async function saveScopeEdit() {
    setScopeSaving(true);
    await updateLeadField(l.id, { quoteScope: scopeDraft });
          ...
          {(() => {
            ...
            return (
              <div className="border border-[#001f97]/20 rounded-lg bg-[#001f97]/[0.03] overflow-hidden">
                ...
                <button
                  type="button"
                  onClick={() => { setScopeOpen((v) => !v); setScopeEditing(false); }}
                  className="w-full flex items-center justify-between px-2.5 py-2 hover:bg-[#001f97]/[0.06] transition-colors cursor-pointer"
                >
                  <span className="text-[10px] font-black text-[#001f97] uppercase tracking-wider">Scope of Work</span>
                  {scopeOpen ? (
                ...
                {/* Expanded body */}
                {scopeOpen && (
                  <div className="px-2.5 pb-2.5 space-y-2 border-t border-[#001f97]/10">
                    {scopeEditing ? (
                      /* Edit mode */
                      ...
                      <div className="pt-2">
                        {items ? (
                          <ul className="space-y-1.5">
                            {items.map((it, i) => (
                              <li key={i} className="flex items-start gap-1.5 text-[11px] text-slate-700 leading-snug">
                                <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-[#001f97] shrink-0" />
                                <span>
                          ...
                          /* Parse quoteScope by bullet markers or newlines */
                          <ul className="space-y-1.5">
                            {(fallback as string)
                              .split(/\s*[•·\-]\s+|\n/)
                              .map((s) => s.trim())
                        ...
                        {/* Edit button (only for quoteScope / fallback text) */}
                        {!items && (
                          <button
                            type="button"
                            onClick={() => { setScopeDraft(fallback as string); setScopeEditing(true); }}
                            className="mt-2 flex items-center gap-1 text-[10px] font-bold text-[#001f97] hover:underline cursor-pointer"
                          >
