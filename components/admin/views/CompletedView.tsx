  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-bold text-slate-600">All Completed Records</span>
              <FileCheck2 className="w-4 h-4 text-[#001f97]" />
            </div>
            <div className="text-2xl font-black text-[#001f97]">{completedLeads.length}</div>
          </button>
          ...
          <button
            ...
            className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
              filterType === "jobs"
                ...
                : "border-slate-200 bg-slate-50/60 hover:border-emerald-500/40"
            }`}
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-bold text-slate-600">Completed Jobs</span>
              <Briefcase className="w-4 h-4 text-emerald-600" />
            </div>
            <div className="text-2xl font-black text-emerald-700">{totalJobsCount}</div>
          </button>
          ...
          <button
            ...
            className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
              filterType === "inspections"
                ...
                : "border-slate-200 bg-slate-50/60 hover:border-sky-500/40"
            }`}
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-bold text-slate-600">Completed Inspections</span>
              <ClipboardList className="w-4 h-4 text-sky-600" />
            </div>
            <div className="text-2xl font-black text-sky-700">{totalInspectionsCount}</div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#e4e9f1] p-5 shadow-xs space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {(search || filterType !== "all") && (
              <button
                ...
                className="text-xs text-rose-600 hover:text-rose-700 font-bold px-2 py-1 rounded-md hover:bg-rose-50 transition-colors cursor-pointer"
              >
                Clear Filters
              </button>
            )}
        ...
        {/* Table header */}
        <div className="grid grid-cols-12 gap-4 xl:gap-6 px-4 py-3 bg-[#e8f0fe] text-[#1e3a8a] text-xs font-black uppercase tracking-wider rounded-xl w-full">
        ...
        <div className="divide-y divide-slate-200/80 w-full">
          {paginated.map((l) => {
            ...
            return (
              <div
                key={l.id}
                className="py-4 px-3 hover:bg-slate-50/70 transition-colors rounded-xl"
              >
                <div className="grid grid-cols-12 gap-4 xl:gap-6 items-start w-full">
                  ...
                  <div className="col-span-4 min-w-0 space-y-2">
                    <div className="flex items-center gap-1.5 text-xs font-bold flex-wrap">
                      <span className="text-[#001f97] whitespace-nowrap font-black">
                        {jobNoDisplay}
                      </span>
                      <span className="text-slate-300 font-bold">|</span>
                      <span
                        className="text-slate-900 font-bold truncate max-w-[180px]"
                  ...
                  <div className="col-span-5 min-w-0 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isInsp && (
                        <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-sky-50 text-sky-800 border border-sky-200">
                          Inspection Done
                        </span>
                      )}
                    </div>
                    ...
                    {/* Assigned Personnel */}
                    <div className="text-xs text-slate-600 space-y-0.5 bg-slate-50 p-2 rounded-lg border border-slate-200/80">
                      {l.technician && (
                        <div className="flex items-center justify-between">
                          <span className="text-slate-500 font-medium">Technician:</span>
                          <span className="font-bold text-slate-800">{l.technician}</span>
                        </div>
                      )}
                      {l.assigned && l.assigned !== l.technician && (
                        <div className="flex items-center justify-between">
                          <span className="text-slate-500 font-medium">Inspector/Staff:</span>
                          <span className="font-bold text-slate-800">{l.assigned}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {l.inspectionReport && (() => {
                      ...
                      return (
                        <div className="p-2 bg-sky-50/60 border border-sky-200 rounded-lg text-xs space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-sky-900 flex items-center gap-1">
                              <ClipboardList className="w-3 h-3 text-sky-600" />
                              Inspection Report
                            </span>
                            <span className="text-[10px] font-black text-sky-700 uppercase bg-sky-100 px-1.5 py-0.5 rounded-full">
                              {l.inspectionReport.status || "Completed"}
                            </span>
                          </div>
                  ...
                  <div className="col-span-3 min-w-0 space-y-1.5">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 py-1.5 px-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                    >
                      <Phone className="w-3.5 h-3.5 text-[#001f97] shrink-0" />
                    ...
                    >
                      ...
                      <span>Photos</span>
                      {photosTotal > 0 && (
                        <span className="min-w-[18px] h-4 px-1 rounded-full bg-blue-100 text-[#001f97] text-[10px] font-black flex items-center justify-center shrink-0">
                          {photosTotal}
                        </span>
                      )}
                    </button>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 py-1.5 px-2.5 bg-sky-50 hover:bg-sky-100 text-sky-800 border border-sky-200 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                    >
                    ...
                    {l.gps && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1.5 py-1.5 px-2.5 bg-[#001f97] hover:bg-[#001777] text-white rounded-lg text-xs font-bold transition-colors cursor-pointer shadow-2xs"
                      >
